...
@section('content')
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
        <form >
          <div class="col-md-9">
            <h1 class="page-header">产品修改</h1>
            <div class="form-group">
              <label for="article-title" class="sr-only">名称</label>
              <input type="text" id="product_name" name="name" class="form-control" placeholder="在此处输入产品名称" value="{{$product['name']}}" required autofocus autocomplete="off">
              <span class="prompt-text" style="color: #db1010"></span>
            </div>
            <div class="form-group">
              <h2 class="add-article-box-title"><span>商品详情</span></h2>
              <script id="product_description" name="content" type="text/plain">{!!$product['description']!!}</script>
            </div>
            <div class="add-article-box">
              <h2 class="add-article-box-title"><span>搜索关键字</span></h2>
              <div class="add-article-box-content">
              	<input type="text" class="form-control" id="keyword" placeholder="请输入关键字" value="{{$product['key_word']}}" name="keyword" autocomplete="off">
              </div>
            </div>
            <div class="add-article-box">
              <h2 class="add-article-box-title"><span>产地</span></h2>
              <div class="add-article-box-content">
                <input type="text" class="form-control"  placeholder="请输入商品原产地" id="origin" value="{{$product['origin']}}" name="origin" autocomplete="off">
              </div>
              <h2 class="add-article-box-title"><span>储存方法</span></h2>
              <div class="add-article-box-content">
                <input type="text" class="form-control"  placeholder="请输入商品储存方法" id="reserve" value="{{$product['reserve']}}" name="reserve" autocomplete="off">
              </div>
              <h2 class="add-article-box-title"><span>保鲜期(天)</span></h2>
              <div class="add-article-box-content">
                <input type="text" class="form-control"  placeholder="请输入商品保鲜期" id="refreshing_time" value="{{$product['refreshing_time']}}" name="refreshing_time" autocomplete="off">
              </div>
              <h2 class="add-article-box-title"><span>营养元素</span></h2>
              <div class="add-article-box-content">
                <input type="text" class="form-control"  placeholder="请输入商品营养元素" id="nourishment" value="{{$product['nourishment']}}" name="nourishment" autocomplete="off">
              </div>
            </div>
          </div>
          <div class="col-md-3">
                ...
                <ul class="category-list">
                  <li>
                    <select name="category" class="form-control category" >
                      @foreach($category->toArray() as $v)
                        <option value="{{$v['id']}}" @if($v['id']==$product['cid'])selected @endif>{{$v['name']}}</option>
                        @endforeach
                      </select>
                  </li>
            ...
            <div class="add-article-box">
              ...
              <div class="add-article-box-content">
                <input type="text" class="form-control" placeholder="请输入特级规格商品价格" id="price_t" value="{{$product['price_t']}}" name="price_t" id="price_t" autocomplete="off">
              </div>
              <div class="add-article-box">
                <h2 class="add-article-box-title"><span>特级规格商品会员价格</span></h2>
                <div class="add-article-box-content">
                  <input type="text" class="form-control" placeholder="请输入特级规格商品会员价格" id="vip_price_t" value="{{$product['vip_price_t']}}" name="vip_price_t" id="vip_price_t" autocomplete="off">
                </div>
            <div class="add-article-box">
              <h2 class="add-article-box-title"><span>库存</span></h2>
              <div class="add-article-box-content">
                <input type="text" class="form-control" placeholder="输入库存量(斤)" id="inventory" value="{{$product['inventory']}}" name="inventory" autocomplete="off">
            </div>
            <div class="add-article-box">
              <h2 class="add-article-box-title"><span>商品图片预览</span></h2>
              <div class="add-article-box-content">
                <img src="{{$product['img_url']}}" id="img_id" height="100" width="100">
              </div>
            ...
            <div class="add-article-box">
              ...
              <div class="add-article-box-content">
                ...
                <select name="status" class="form-control status" >
                    ...
                    <option value="1" @if($product['status']=='1')selected @endif>正常</option>
                    <option value="2" @if($product['status']=='2')selected @endif>促销</option>
                    <option value="3" @if($product['status']=='3')selected @endif>下架</option>
                </select>
              </div>
              <div class="add-article-box-footer">
